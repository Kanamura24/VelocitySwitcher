...
velocityswitcher_spec = ["implementation_id", "VelocitySwitcher", 
		 "type_name",         "VelocitySwitcher", 
...
# 
# 
class VelocitySwitcher(OpenRTM_aist.DataFlowComponentBase):
	...
	def __init__(self, manager):
		...
		"""
		"""
		self._vel_inIn = OpenRTM_aist.InPort("vel_in", self._d_vel_in)
		self._d_vel_out = OpenRTM_aist.instantiateDataType(RTC.TimedVelocity2D)
		"""
		"""
		self._vel_outOut = OpenRTM_aist.OutPort("vel_out", self._d_vel_out)
		...
		"""
		"""
		self._gameStateSwitcherService = VelocitySwitcherService_i()
		"""
		"""
	...
	def onInitialize(self):
		...
		# Set InPort buffers
		self.addInPort("vel_in",self._vel_inIn)
		
		# Set OutPort buffers
		self.addOutPort("vel_out",self._vel_outOut)
		
		# Set service provider to Ports
		self._gameStateSwitcherPort.registerProvider("KanamuraRobotics::VelocitySwitcherService", "KanamuraRobotics::VelocitySwitcherService", self._gameStateSwitcherService)
...
def VelocitySwitcherInit(manager):
    profile = OpenRTM_aist.Properties(defaults_str=velocityswitcher_spec)
    manager.registerFactory(profile,
                            VelocitySwitcher,
                            OpenRTM_aist.Delete)

def MyModuleInit(manager):
    VelocitySwitcherInit(manager)

    # Create a component
    comp = manager.createComponent("VelocitySwitcher")
